'use client'
import Playground from "@/components/prompt-editor/playground"
import {
  EditorState,
  convertFromRaw,
} from "draft-js"
...
if (typeof window !== 'undefined') {
  console.log('*************YESSSSS**************', window);

  try {
    SETTINGS = JSON.parse(window.localStorage.getItem("app_settings"));
...
const saveSettings = () => {
  ...
  localStorage.setItem("app_settings", _settings)
}
...
const APIContextWrapper = ({ children }) => {
  ...
  const fetchWrapper = function (url, options = {}) {
    return fetch(`http://localhost:5432${url}`, options)
  }


  const Model = {
    getAll: async () => (await fetchWrapper("/api/models")).json(),
    getAllEnabled: async () => (await fetchWrapper("/api/models-enabled")).json(),
    toggle: async (provider, model) => (await fetchWrapper(`/api/provider/${provider}/model/${encodeURIComponent(model)}/toggle-status`)).json(),
    search: async (provider, query) => (await fetchWrapper(`/api/provider/${provider}/models/search?query=${query}`)).json(),
  };


  const Provider = {
    setAPIKey: async (provider, apiKey) => (await fetchWrapper(`/api/provider/${provider}/api-key`, {
      method: "PUT", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ apiKey: apiKey })
    })).json(),
    getAll: async () => await fetchWrapper("/api/providers").json(),
    getAllWithModels: async () => await fetchWrapper("/api/providers-with-key-and-models").json(),
  };
  ...
  function createTextCompletionRequest({ prompt, models }) {
    const url = "http://localhost:5432/api/inference/text/stream";
    const payload = {
      prompt: prompt,
      models: models,
    };
  ...
  function createChatCompletionRequest(prompt, model) {
    const url = "http://localhost:5432/api/inference/chat/stream";
...
const PlaygroundContextWrapper = ({ page, children }) => {
  ...
  const updateModelsData = async () => {
    ...
    for (const [model_key, modelDetails] of Object.entries(json_params)) {
      ...
      console.log(model_key, modelDetails, "model_key, modelDetails");
