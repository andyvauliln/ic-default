import { ModelsContext, ModelsStateContext, ParametersContext } from "@/app/build/building-prompt/page"
import { useBreakpoint } from "@/hooks/use-breakpoint"
import { handleSelectModel } from "@/utils/helpers"
import { BarChart2, Copy, Filter, Trash2 } from "lucide-react"
    ...
    const generate_card = (modelState: any) => {
        return (
            <div
                ...
                className={`relative select-none my-2 flex justify-center items-center rounded-md border border-slate-200 font-mono text-sm dark:border-slate-700 overflow-hidden ${modelState.selected ? "bg-slate-200 dark:bg-slate-200" : ""
                    } ${modelState.enabled
            ...
            >
                <div
                    className={`pl-4 py-3 flex-1 overflow-hidden ${!modelState.enabled ? "text-zinc-400" : ""
                        }`}
                    onClick={(event) => {
                    ...
                    <Tooltip delayDuration={300} skipDelayDuration={150}>
                        <TooltipTrigger asChild>
                            <div
                                ...
                                className={`mx-1 cursor-pointer flex justify-center items-center w-[24px] h-[24px] rounded-full border-[1px] border-slate-200 select-none ${parametersContext.showParametersTable
